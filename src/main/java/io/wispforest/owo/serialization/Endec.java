...
import io.wispforest.owo.serialization.Serializer.Sequence;
...
public interface Endec<T> {
    ...
    static <T> Endec<T> ofCodec(Codec<T> codec) {
        return of(
                (serializer, value) -> EdmEndec.INSTANCE.encode(serializer, Util.getOrThrow(codec.encodeStart(EdmOps.INSTANCE, value), IllegalStateException::new)),
                deserializer -> Util.getOrThrow(codec.parse(EdmOps.INSTANCE, EdmEndec.INSTANCE.decode(deserializer)), IllegalStateException::new)
        );
    }
